...
--template-body file://cf_templates/$CF_TEMPLATE \
--parameters \
ParameterKey=PeerVPC,ParameterValue="vpc-e9335a92" \
ParameterKey=PeerVPCOwner,ParameterValue=\"$BridgeDevAwsAccountId\" \
ParameterKey=PeerVPCCIDR,ParameterValue="172.51.0.0/16" \
ParameterKey=PeerRoleName,ParameterValue="bridge-VPCPeeringAuthorizerRole-13KSQX7XGYVAJ""
message=$($UPDATE_CMD 2>&1 1>/dev/null)
error_code=$(echo $?)
if [[ $error_code -ne 0 && $message =~ .*"No updates are to be performed".* ]]; then
...
--template-body file://cf_templates/$CF_TEMPLATE \
--parameters \
ParameterKey=PeerVPC,ParameterValue="vpc-1362e468" \
ParameterKey=PeerVPCOwner,ParameterValue=\"$SynapseDevAwsAccountId\" \
ParameterKey=PeerVPCCIDR,ParameterValue="10.11.0.0/16" \
ParameterKey=PeerRoleName,ParameterValue="essentials-VPCPeeringAuthorizerRole-ANWU0AZR9EZ7""
message=$($UPDATE_CMD 2>&1 1>/dev/null)
error_code=$(echo $?)
if [[ $error_code -ne 0 && $message =~ .*"No updates are to be performed".* ]]; then
  echo -e "\nNo stack changes detected. An update is not required."
